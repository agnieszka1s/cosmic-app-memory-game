...
const flipCard = function() {
  ...
  if (flippedCards.length === 0) {
    ...
    setTimeout(function() {
      const theSame =
        flippedCards[0].dataset.pair === flippedCards[1].dataset.pair;
      if (theSame) {
        flippedCards.forEach((element) => element.classList.add('match'));
        result++;
        showResult();

        cards = cards.filter((element) => !element.classList.contains('match'));
        ...
        flippedCards.forEach((element) => element.classList.add('back'));
      }
